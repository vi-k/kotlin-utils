...
При настройке тега блоки `characterStyle`, `paragraphStyle` и `borderStyle` не выполняются сразу для получения готового стиля. Они будут запущены позже (по аналогии с модификатором `lazy`) во время парсинга, когда тег встретится в тексте. Неявным параметром `this` в эти блоки (это обычные лямбды) передаётся сам стиль, а явным параметром (по-умолчанию `it`) передаётся тег, который хранит в себе весь список атрибутов, полученных из HTML. По этим атрибутам мы можем настроить наши теги:
...
Это "хардкод". Тот же результат можно получить, сделав описание атрибутов:
...
Результат в итоге будет тот же самый. Отличия лишь в том, что в первом случае проверяется каждый из возможных атрибутов, а во втором случае работа происходит только с атрибутами, реально присутствующими в HTML-тексте.

При таком варианте объявления внутри блока `tag` в стилях устанавливается значение по-умолчанию, а в блоках `attr` на это значение уже накладывается то, что установлено пользователем в HTML-тексте.

Объявление атрибутов внутри блока `tag` касается только текущего тега и на другой тег (в нашем примере это `span`) не влияет.

Блоки стилей `characterStyle`, `borderStyle` и `paragraphStyle` в описании `attr` это лямбды с отложенным (`lazy`) запуском, существующие в двух вариантах:
...
Атрибут может быть объявлен и вне блока `tag`, тогда вторым параметром надо будет указать название тега, к которому будет применено объявление атрибута.
